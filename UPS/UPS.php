<?php
    ...
    class UPS {
        ...
        function setRateRequest( $requestVar ) {
            if($requestVar["public"]=='02'){
                $this->isProduction = false;
            }
            require_once dirname(__file__) . "\Request\multiRequest.php";
            $rateRequest = new RateRequest();
            $rateRequest->setRequest($requestVar["context"]);
            $rateRequest->setShipper($rateRequest->getShipperInfo());
            $rateRequest->setShipmentByType("ShipTo", $rateRequest->getShipToInfo($requestVar["soNum"]));
            $rateRequest->setShipmentByType("ShipFrom", $rateRequest->getShipFromInfo());
            $rateRequest->setService($rateRequest->getServiceInfo_UI($requestVar["shipMethod"]));
            $rateRequest->setPackage($rateRequest->getPackageInfo($requestVar["sku"]));
