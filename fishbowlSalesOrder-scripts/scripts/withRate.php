<?php
//error_reporting(0); 
    include_once "models/FBSalesOrder.php";
    require_once "models/searchOrder.php";
    include_once "/../../UPS/UPS.php";
    
    define('ScSKU_INI', "models/scSku_map.ini");
    define('KiT_INI', "models/kit_map.ini");
    define('FiSHBOWL_INI', "models/fishbowl.ini");
   ...
   foreach($oneOrder as $key => $value){
        $scOrder = $scOrderObj->getSCOrder($value);
               $orderItems = $scOrder['OrderItems'];
                ...
                $sku = isset($scSku['SC-FB'][$orderItems['OrderItem']['SKU']])?$scSku['SC-FB'][$orderItems['OrderItem']['SKU']]:$orderItems['OrderItem']['SKU'];
                echo $sku;
                $requestVar=array(
                "context" =>'test',
                "soNum" =>$value,
        ...
        //$totalPrice=$ne_price*$orderItems['OrderItem']['Qty'];
        print_r($ne_price);     
       
       
        //$rate->setRateRequest($requestVar);
       // $result = $rate->invokeRateRequest();
        //$price = $result->RateResponse->RatedShipment->NegotiatedRateCharges->TotalCharge->MonetaryValue;
       
       /*$fbSalesOrder->fbApi->getProducts('Get', $value, 0, null) ;
            ...
            echo "<pre>";
            print_r($fbSalesOrder->fbApi->result['FbiMsgsRs']['ProductGetRs']['Product']);
            echo "</pre>";*/
        ...
        //check if it is a kit product        
            ...
            if(isset($kitSku[$orderItems['OrderItem']['SKU']])){
                ...
                $SalesOrderItem = $Items->addChild('SalesOrderItem');
                $SalesOrderItem->addChild('Flag', true);
                $SalesOrderItem->addChild('ProductNumber', $fbSalesOrder->getProductNum());
                $SalesOrderItem->addChild('Taxable', $fbSalesOrder->getTaxable());
                $SalesOrderItem->addChild('KitItem', 'false');
                $SalesOrderItem->addChild('ShowItem', 'true');
                $SalesOrderItem->addChild('Quantity', $fbSalesOrder->getQuantity());
                $SalesOrderItem->addChild('ProductPrice', $fbSalesOrder->getProductPrice());
                $SalesOrderItem->addChild('UOMCode', $fbSalesOrder->getUOMCode());
                $SalesOrderItem->addChild('ItemType', $fbSalesOrder->getItemType());
                ...
                foreach($kitSku[$orderItems['OrderItem']['SKU']] as $kit =>$kit_value){
                    ...
                    $SalesOrderItem = $Items->addChild('SalesOrderItem');
                    $SalesOrderItem->addChild('Flag', false);
                    $SalesOrderItem->addChild('ProductNumber', $fbSalesOrder->getProductNum());
                    $SalesOrderItem->addChild('KitItem', 'ture');
                    $SalesOrderItem->addChild('Taxable', $fbSalesOrder->getTaxable());
                    $SalesOrderItem->addChild('Quantity', $fbSalesOrder->getQuantity());
                    $SalesOrderItem->addChild('ProductPrice', $fbSalesOrder->getProductPrice());
                    $SalesOrderItem->addChild('UOMCode', $fbSalesOrder->getUOMCode());
        ...
        if($scOrder['ShipFee']==0){
            ...
            $SalesOrderItem->addChild('Taxable', $fbSalesOrder->getTaxable());
            $SalesOrderItem->addChild('Quantity', $fbSalesOrder->getQuantity());
            $SalesOrderItem->addChild('KitItemFlag', 'false');
            $SalesOrderItem->addChild('ProductPrice', $fbSalesOrder->getProductPrice());
            $SalesOrderItem->addChild('UOMCode', $fbSalesOrder->getUOMCode());
            $SalesOrderItem->addChild('ItemType', $fbSalesOrder->getItemType());
            $SalesOrderItem->addChild('QuickBooksClassName', $fbSalesOrder->getProductQuickBooksClassName());
        ...
        $fbSalesOrder->setTaxable('false');
        $fbSalesOrder->setQuantity(1);
        $fbSalesOrder->setProductPrice(99);
        $fbSalesOrder->setUOMCode('ea');
        $fbSalesOrder->setItemType('60');
        $fbSalesOrder->setProductQuickBooksClassName(' ');
        $fbSalesOrder->setNewItemFlag('false');
        ...
        try{$fbSalesOrder->saveSO($salesOrderXml);}
        catch( Exception $e) {
        var_dump( $e );
    }
